 ...
 * @author MAGM
 */
public class MAFPopulation_rcLS1 {
    ...
    public MAFPopulation_rcLS1(String arc) {
        this.archivo = arc;
    }
...
public void MF(int[][] mf)
{
    ...
    public static void main(String[] args) throws FileNotFoundException, IOException {
        ...
        double besttime=0;
        double timelimit=0.003506398;
        Scanner teclado = new Scanner(System.in);
        MAFPopulation_rcLS1 obj=new MAFPopulation_rcLS1("10n20m.txt");
        obj.readFile(obj.archivo);
        int[][] mf=new int[obj.M][obj.N];
        int[][] rl=new int[30][obj.N];
        int[] bestlocal=new int[obj.N];
        for(int z=0;z<30;z++)
        {
            for(int zz=0;zz<obj.N;zz++)
            {
                rl[z][zz]=0;
             }
        }
...
while(contando<=30)
{
        ...
        while (obj.tiempo<timelimit)//0.001840843  
        {
            ...
            bestlocal=obj.mejorsol;            
//System.arraycopy(obj.mejorsol, 0, bestlocal, 0, obj.mejorsol.length);
            for(int k=0;k<bestlocal.length;k++)
            {
                rl[contando-1][k]=bestlocal[k];
            }
...
for(int i=0;i<obj.M;i++){
    for(int ii=0;ii<obj.N;ii++){
        System.out.print(obj.ME[i][ii]+";");}
    System.out.println(' ');
}
}
}